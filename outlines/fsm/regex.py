...
def create_fsm_index_end_to_end_using_rustlines_state_scan_tokens(
    fsm_info: FSMInfo,
    vocabulary: Dict[str, List[int]],
    ...
    while next_states:
        ...
        token_ids_end_states = rustlines.state_scan_tokens(
            dict(fsm_info.transitions),
            dict(fsm_info.alphabet_symbol_mapping),
...
def create_fsm_index_end_to_end(
    ...
    """Create an FSM state-to-vocabulary map/index through end-to-end token parsing."""
    res = rustlines.create_fsm_index_end_to_end_rust(
        dict(fsm_info.transitions),
        dict(fsm_info.alphabet_symbol_mapping),
        fsm_info.alphabet_anything_value,
        fsm_info.initial,
...
# The original implementation of `create_fsm_index_end_to_end` is below (renamed)
def original_create_fsm_index_end_to_end(
    fsm_info: FSMInfo,
    vocabulary: Dict[str, List[int]],
) -> Dict[int, Set[Tuple[int, int]]]:
    """Create an FSM state-to-vocabulary map/index through end-to-end token parsing."""
